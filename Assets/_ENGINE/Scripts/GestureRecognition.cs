...
namespace XREngine.VR.HandTracking
{
    public class GestureRecognition : MonoBehaviour
    {
        ...
        public static event Action OnRightHandIndexPinching = delegate { };
        public static event Action OnRightHandIndexNotPinching = delegate { };
        ...
        private bool leftHandIndexPinching;
        private bool rightHandIndexPinching;
        ...
        private void PinchRecognition()
        {
            ...
            //if (leftHand.GetFingerIsPinching(OVRHand.HandFinger.Index) && leftHand.GetFingerIsPinching(OVRHand.HandFinger.Middle)
            //    && leftHand.GetFingerIsPinching(OVRHand.HandFinger.Ring))// && leftHand.GetFingerIsPinching(OVRHand.HandFinger.Pinky))

            if (leftHand.GetFingerIsPinching(OVRHand.HandFinger.Index) && !leftHandIndexPinching)
            {
            ...
            //else if (!leftHand.GetFingerIsPinching(OVRHand.HandFinger.Index) && leftHand.GetFingerIsPinching(OVRHand.HandFinger.Middle)
            //   && leftHand.GetFingerIsPinching(OVRHand.HandFinger.Ring) && leftHand.GetFingerIsPinching(OVRHand.HandFinger.Pinky))
            //{
            //    SetText("Left Hand Showing Index Finger");
            //}

            // Right Hand
            if (rightHand.GetFingerIsPinching(OVRHand.HandFinger.Index) & !rightHandIndexPinching)
            {
                rightHandIndexPinching = true;
                OnRightHandIndexPinching();
                SetText("Right Hand Pinching Using Index Finger");
            }
            else if (!rightHand.GetFingerIsPinching(OVRHand.HandFinger.Index) & rightHandIndexPinching)
            {
                rightHandIndexPinching = false;
                OnRightHandIndexNotPinching();
                SetText("");
            }
