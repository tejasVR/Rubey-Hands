...
namespace HighlightPlus {
    ...
    public partial class HighlightEffect : MonoBehaviour {
        ...
        // in URP, this event gets also called
        //void OnRenderObject() {
        //    if (usingPipeline) return;
        //    BuildCommandBuffer(Camera.current);
        //    Graphics.ExecuteCommandBuffer(cbHighlight);
        //}
        ...
        void BuildCommandBuffer(Camera cam) {
                        ...
                        for (int glowPass = 0; glowPass < glowPasses.Length; glowPass++) {
                            if (glowQuality == QualityLevel.High) {
                                for (int o = 0; o < offsets.Length; o++) {
                                    ...
                                    fxMatGlow[matIndex].SetColor("_GlowColor", glowPasses[glowPass].color);
                                    fxMatGlow[matIndex].SetVector("_Glow", new Vector4(fade * glowReal * glowPasses[glowPass].alpha, glowPasses[glowPass].offset * glowWidth / 100f, glowMagicNumber1, glowMagicNumber2));
            ...
            if (useSmoothBlend && _highlighted) {
